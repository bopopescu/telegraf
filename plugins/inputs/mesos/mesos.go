...
var allMetrics = map[Role][]string{
	MASTER: {"resources", "main", "system", "agents", "frameworks", "tasks", "messages", "evqueue", "registrar"},
...
func (m *Mesos) Gather(acc telegraf.Accumulator) error {
	...
	for _, subordinate := range m.subordinateURLs {
		...
		// if !m.SubordinateTasks {
		// 	continue
		// }

		// wg.Add(1)
		// go func(c string) {
		// 	acc.AddError(m.gatherSubordinateTaskMetrics(subordinate, acc))
		// 	wg.Done()
		// 	return
		// }(v)
	}
...
// mainBlocks serves as kind of metrics registry groupping them in sets
